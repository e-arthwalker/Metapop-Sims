...
getwd()
setwd("C:/Users/Administrator/Desktop/Final Metapop R Package/changing to mean min/50 patches") #access working directory that all my metapop functions are stored in
getwd()
...
parallel.alphas<-function(j, n.reps, k){
  ...
  regular.data<-vary.alpha.in.parallel(alphas=alpha.data[1,j+1], landscape.type="regular", n.landscapes=n.reps, k=k, n.patches=50, landscape.limit=100, no.runs=500, scaled.lm=100)
  random.data<-vary.alpha.in.parallel(alphas=alpha.data[2,j+1], landscape.type="random", n.landscapes=n.reps, k=k, n.patches=50, landscape.limit=100, no.runs=500, scaled.lm=100)
  clustered.data<-vary.alpha.in.parallel(alphas=alpha.data[3,j+1], landscape.type="clustered", n.landscapes=n.reps, k=k, n.patches=50, landscape.limit=100, no.runs=500, scaled.lm=100)
...
#didn't work for 100 but worked for 20
